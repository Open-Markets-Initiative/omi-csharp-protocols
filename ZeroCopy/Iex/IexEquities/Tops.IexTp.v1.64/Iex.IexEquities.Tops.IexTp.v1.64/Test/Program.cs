...
using Pcap.CSharp;
using Iex.IexEquities.Tops.IexTp.Testing;
using static Iex.IexEquities.Tops.IexTp.Testing.TestHarness;

return Run<MessageCode>(args, "Iex.IexEquities.Tops.IexTp.v1.64.Test", ReadMessages, PrintMessage);
...
static void PrintMessage(MessageCode type, ReadOnlySpan<byte> payload, int seq)
{
    switch (type)
    {
        ...
        {
            var msg = MemoryMarshal.Read<QuoteUpdateMessage.Layout>(payload);
            Console.WriteLine($"[{seq}] QuoteUpdate: timestamp={FormatTs(msg.Timestamp.Value)} symbol={msg.Symbol.Value,-8} bidPrice={msg.BidPrice.Value,10} askPrice={msg.AskPrice.Value,10}");
            break;
        }
