...
/// <summary>
///  Represents the OfficialPriceMessage from the Deep protocol.
/// </summary>
/// <remarks>
...
public sealed class OfficialPriceMessage : IMessage, IEquatable<OfficialPriceMessage>
{
    /// <summary>
    ///  Message type identifier.
    /// </summary>
    public const char MessageType = 'X';

    /// <summary>
    ///  Total fixed byte length of this message.
    /// </summary>
    private const int MessageByteLength = 25;

    /// <summary>
    ///  Price type identifier
    /// </summary>
    public PriceTypeField PriceType { get; set; } = new();
    ...
    /// </summary>
    public OfficialPriceField OfficialPrice { get; set; } = new();
    ...
    /// <summary>
    ///  The total fixed byte length of this message (25 bytes).
    /// </summary>
    ...
    /// <summary>
    ///  Initializes a new <see cref="OfficialPriceMessage"/> with all fields at their default values.
    /// </summary>
    public OfficialPriceMessage() { }

    /// <summary>
    ///  Initializes a new <see cref="OfficialPriceMessage"/> from an existing instance, copying all field values.
    /// </summary>
    /// <param name="other">The source instance to copy from.</param>
    public OfficialPriceMessage(OfficialPriceMessage other) : this()
    {
        ArgumentNullException.ThrowIfNull(other);
        if (other.Raw is { } raw) Raw = raw.ToArray();
        IsDecoded = other.IsDecoded;
        PriceType.CopyFrom(other.PriceType);
        Timestamp.CopyFrom(other.Timestamp);
        Symbol.CopyFrom(other.Symbol);
        OfficialPrice.CopyFrom(other.OfficialPrice);
    }

    /// <summary>
    ///  Parses a <see cref="OfficialPriceMessage"/> from a binary data span at the given offset.
    ...
    public OfficialPriceMessage(ReadOnlySpan<byte> data, ref int offset)
    {
        var start = offset;
        var decoded = true;
    ...
    /// <summary>
    ///  Parses a <see cref="OfficialPriceMessage"/> from a binary data span.
    /// </summary>
    /// <param name="data">The raw message bytes. Must contain at least 25 bytes.</param>
    /// <returns>A new <see cref="OfficialPriceMessage"/> with all fields populated from the binary data.</returns>
    public static OfficialPriceMessage Parse(ReadOnlySpan<byte> data)
    {
        var offset = 0;
        return new OfficialPriceMessage(data, ref offset);
    }

    /// <summary>
    ///  Parses binary data into this <see cref="OfficialPriceMessage"/> instance in place, reusing existing field objects.
    /// </summary>
    /// <param name="data">The raw message bytes. Must contain at least 25 bytes.</param>
    ...
    /// <summary>
    ///  Encodes this <see cref="OfficialPriceMessage"/> into a binary data span starting at the given offset.
    /// </summary>
    /// <param name="data">The destination span. Must be at least offset + 25 bytes.</param>
    ...
    {
        offset = PriceType.Encode(data, offset);
        offset = Timestamp.Encode(data, offset);
        offset = Symbol.Encode(data, offset);
        offset = OfficialPrice.Encode(data, offset);
        return offset;
    }

    /// <summary>
    ///  Encodes this <see cref="OfficialPriceMessage"/> into a binary data span starting at offset 0.
    /// </summary>
    /// <param name="data">The destination span. Must be at least 25 bytes.</param>
    /// <returns>The offset after the last written byte.</returns>
    public int Encode(Span<byte> data) => Encode(data, 0);

    /// <summary>
    ///  Returns a <see cref="OfficialPriceMessage"/> with deterministic sample values in every field,
    ///  suitable for round-trip encoding tests.
    /// </summary>
    public static OfficialPriceMessage Sample()
    {
        var msg = new OfficialPriceMessage();
        msg.PriceType = PriceTypeField.Sample(seed: 0);
        msg.Timestamp = TimestampField.Sample(seed: 1);
        msg.Symbol = SymbolField.Sample(seed: 2);
        msg.OfficialPrice = OfficialPriceField.Sample(seed: 3);
        return msg;
    }
    ...
    /// <summary>
    ///  Resets all fields of this <see cref="OfficialPriceMessage"/> to their default values and clears the raw backing store.
    /// </summary>
    public void Clear()
    {
        Raw = null;
    ...
    /// <summary>
    ///  Compares this <see cref="OfficialPriceMessage"/> to another using field-by-field comparison.
    /// </summary>
    public bool Equals(OfficialPriceMessage? other)
    {
        if (other is null) return false;
        if (!PriceType.Equals(other.PriceType)) return false;
        if (!Timestamp.Equals(other.Timestamp)) return false;
        if (!Symbol.Equals(other.Symbol)) return false;
        if (!OfficialPrice.Equals(other.OfficialPrice)) return false;
        return true;
    }

    /// <summary>
    ///  Compares two <see cref="OfficialPriceMessage"/> instances field by field.
    /// </summary>
    public override bool Equals(object? obj)
    {
        return obj is OfficialPriceMessage other && Equals(other);
    }
    ...
    /// <summary>
    ///  Returns a hash code for this <see cref="OfficialPriceMessage"/> by combining the hash of each field and counted-group container.
    /// </summary>
    public override int GetHashCode()
    {
        ...
        hash.Add(Timestamp);
        hash.Add(Symbol);
        hash.Add(OfficialPrice);
        return hash.ToHashCode();
    }

    public static bool operator ==(OfficialPriceMessage? a, OfficialPriceMessage? b) => ReferenceEquals(a, b) || (a is not null && a.Equals(b));
    public static bool operator !=(OfficialPriceMessage? a, OfficialPriceMessage? b) => !(a == b);
    ...
    public readonly struct Changes : IEquatable<Changes>
    {
        ...
        public override string ToString()
        {
            ...
            var first = true;
            if (Has(0)) { if (!first) sb.Append(", "); sb.Append("PriceType"); first = false; }
            if (Has(1)) { if (!first) sb.Append(", "); sb.Append("Timestamp"); first = false; }
            if (Has(2)) { if (!first) sb.Append(", "); sb.Append("Symbol"); first = false; }
            if (Has(3)) { if (!first) sb.Append(", "); sb.Append("OfficialPrice"); first = false; }
            return sb.ToString();
        }
    }
    ...
    public Changes Diff(OfficialPriceMessage other)
    {
        ArgumentNullException.ThrowIfNull(other);
        var diff = Changes.None;
        if (!PriceType.Equals(other.PriceType)) diff = diff.With(0);
        if (!Timestamp.Equals(other.Timestamp)) diff = diff.With(1);
        if (!Symbol.Equals(other.Symbol)) diff = diff.With(2);
        if (!OfficialPrice.Equals(other.OfficialPrice)) diff = diff.With(3);
        return diff;
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(other);
        if (other is not OfficialPriceMessage typed) throw new ArgumentException("DiffFields requires same message type", nameof(other));
    ...
    public IEnumerable<(string Name, string Before, string After)> DiffReport(OfficialPriceMessage other)
    {
        ArgumentNullException.ThrowIfNull(other);
        var changes = Diff(other);
        if (changes.IsNone) yield break;
        if (changes.Has(0)) yield return (PriceType.Name, PriceType.ToFormattedString(), other.PriceType.ToFormattedString());
        ...
        if (changes.Has(3)) yield return (OfficialPrice.Name, OfficialPrice.ToFormattedString(), other.OfficialPrice.ToFormattedString());
    }
    ...
    {
        FieldName.PriceType => PriceType,
        FieldName.Timestamp => Timestamp,
        FieldName.Symbol => Symbol,
        FieldName.OfficialPrice => OfficialPrice,
        _ => throw new ArgumentOutOfRangeException(nameof(field)),
    };

    /// <summary>
    ///  Appends a human-readable representation of all fields in this <see cref="OfficialPriceMessage"/> to <paramref name="builder"/>,
    ...
    public void ToFormattedString(StringBuilder builder, PrintOptions options = default)
    {
        ...
        builder.Append(indent).AppendLine("OfficialPriceMessage:");
        foreach (var f in Fields)
        {
    ...
    /// <summary>
    ///  Returns a human-readable representation of all fields in this <see cref="OfficialPriceMessage"/>,
    ...
    public string ToFormattedString(PrintOptions options = default)
    {
        if (options.IsIdentifier)
            return $"[{MessageType}] [{PriceType.Value}] [{Timestamp.Value}] [{Symbol.Value}]";
        if (options.IsCompact)
        {
            var sb = new StringBuilder();
            sb.Append("OfficialPriceMessage { ");
            bool first = true;
            foreach (var f in Fields)
            {
                if (!first) sb.Append(", ");
    ...
    /// <summary>
    ///  Returns a compact one-line representation of all fields in this <see cref="OfficialPriceMessage"/>.
    /// </summary>
    public string ToCompact() => ToFormattedString(PrintOptions.Compact);

    /// <summary>
    ///  Returns a human-readable representation of this <see cref="OfficialPriceMessage"/>.
    /// </summary>
