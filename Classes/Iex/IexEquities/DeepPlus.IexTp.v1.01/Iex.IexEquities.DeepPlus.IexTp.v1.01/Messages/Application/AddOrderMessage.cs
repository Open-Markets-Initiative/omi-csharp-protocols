...
/// <summary>
///  Represents the AddOrderMessage from the DeepPlus protocol.
/// </summary>
/// <remarks>
///  A displayed order that has been added to the IEX Book
/// </remarks>
[DebuggerDisplay("{ToFormattedString(PrintOptions.Identifier),nq}")]
public sealed class AddOrderMessage : IMessage, IEquatable<AddOrderMessage>
{
    /// <summary>
    ...
    ///  Side of order
    /// </summary>
    public SideField Side { get; set; } = new();
    ...
    /// </summary>
    public OrderIdField OrderId { get; set; } = new();
    ...
    /// <summary>
    ///  Initializes a new <see cref="AddOrderMessage"/> with all fields at their default values.
    /// </summary>
    public AddOrderMessage() { }

    /// <summary>
    ///  Initializes a new <see cref="AddOrderMessage"/> from an existing instance, copying all field values.
    /// </summary>
    /// <param name="other">The source instance to copy from.</param>
    public AddOrderMessage(AddOrderMessage other) : this()
    {
        ArgumentNullException.ThrowIfNull(other);
        if (other.Raw is { } raw) Raw = raw.ToArray();
        IsDecoded = other.IsDecoded;
        Side.CopyFrom(other.Side);
    ...
    /// <summary>
    ///  Parses a <see cref="AddOrderMessage"/> from a binary data span at the given offset.
    ...
    public AddOrderMessage(ReadOnlySpan<byte> data, ref int offset)
    {
        var start = offset;
        var decoded = true;
    ...
    /// <summary>
    ///  Parses a <see cref="AddOrderMessage"/> from a binary data span.
    /// </summary>
    /// <param name="data">The raw message bytes. Must contain at least 37 bytes.</param>
    /// <returns>A new <see cref="AddOrderMessage"/> with all fields populated from the binary data.</returns>
    public static AddOrderMessage Parse(ReadOnlySpan<byte> data)
    {
        var offset = 0;
        return new AddOrderMessage(data, ref offset);
    }

    /// <summary>
    ///  Parses binary data into this <see cref="AddOrderMessage"/> instance in place, reusing existing field objects.
    /// </summary>
    /// <param name="data">The raw message bytes. Must contain at least 37 bytes.</param>
    ...
    /// <summary>
    ///  Encodes this <see cref="AddOrderMessage"/> into a binary data span starting at the given offset.
    /// </summary>
    ...
    /// <summary>
    ///  Encodes this <see cref="AddOrderMessage"/> into a binary data span starting at offset 0.
    /// </summary>
    ...
    /// <summary>
    ///  Returns a <see cref="AddOrderMessage"/> with deterministic sample values in every field,
    ///  suitable for round-trip encoding tests.
    /// </summary>
    public static AddOrderMessage Sample()
    {
        var msg = new AddOrderMessage();
        msg.Side = SideField.Sample(seed: 0);
        msg.Timestamp = TimestampField.Sample(seed: 1);
        msg.Symbol = SymbolField.Sample(seed: 2);
        msg.OrderId = OrderIdField.Sample(seed: 3);
    ...
    /// <summary>
    ///  Resets all fields of this <see cref="AddOrderMessage"/> to their default values and clears the raw backing store.
    /// </summary>
    public void Clear()
    {
        Raw = null;
    ...
    /// <summary>
    ///  Compares this <see cref="AddOrderMessage"/> to another using field-by-field comparison.
    /// </summary>
    public bool Equals(AddOrderMessage? other)
    {
        if (other is null) return false;
        if (!Side.Equals(other.Side)) return false;
    ...
    /// <summary>
    ///  Compares two <see cref="AddOrderMessage"/> instances field by field.
    /// </summary>
    public override bool Equals(object? obj)
    {
        return obj is AddOrderMessage other && Equals(other);
    }
    ...
    /// <summary>
    ///  Returns a hash code for this <see cref="AddOrderMessage"/> by combining the hash of each field and counted-group container.
    /// </summary>
    ...
    public static bool operator ==(AddOrderMessage? a, AddOrderMessage? b) => ReferenceEquals(a, b) || (a is not null && a.Equals(b));
    public static bool operator !=(AddOrderMessage? a, AddOrderMessage? b) => !(a == b);
    ...
    public Changes Diff(AddOrderMessage other)
    {
        ArgumentNullException.ThrowIfNull(other);
        var diff = Changes.None;
        if (!Side.Equals(other.Side)) diff = diff.With(0);
        if (!Timestamp.Equals(other.Timestamp)) diff = diff.With(1);
        if (!Symbol.Equals(other.Symbol)) diff = diff.With(2);
        if (!OrderId.Equals(other.OrderId)) diff = diff.With(3);
    ...
    {
        ArgumentNullException.ThrowIfNull(other);
        if (other is not AddOrderMessage typed) throw new ArgumentException("DiffFields requires same message type", nameof(other));
    ...
    public IEnumerable<(string Name, string Before, string After)> DiffReport(AddOrderMessage other)
    {
        ArgumentNullException.ThrowIfNull(other);
        var changes = Diff(other);
        if (changes.IsNone) yield break;
        if (changes.Has(0)) yield return (Side.Name, Side.ToFormattedString(), other.Side.ToFormattedString());
        ...
        if (changes.Has(3)) yield return (OrderId.Name, OrderId.ToFormattedString(), other.OrderId.ToFormattedString());
    ...
    /// <summary>
    ///  Appends a human-readable representation of all fields in this <see cref="AddOrderMessage"/> to <paramref name="builder"/>,
    ...
    public void ToFormattedString(StringBuilder builder, PrintOptions options = default)
    {
        ...
        builder.Append(indent).AppendLine("AddOrderMessage:");
        foreach (var f in Fields)
        {
    ...
    /// <summary>
    ///  Returns a human-readable representation of all fields in this <see cref="AddOrderMessage"/>,
    ...
    public string ToFormattedString(PrintOptions options = default)
    {
        if (options.IsIdentifier)
            return $"[{MessageType}] [{Side.Value}] [{Timestamp.Value}] [{Symbol.Value}]";
        if (options.IsCompact)
        {
            var sb = new StringBuilder();
            sb.Append("AddOrderMessage { ");
            bool first = true;
            foreach (var f in Fields)
            {
                if (!first) sb.Append(", ");
    ...
    /// <summary>
    ///  Returns a compact one-line representation of all fields in this <see cref="AddOrderMessage"/>.
    /// </summary>
    public string ToCompact() => ToFormattedString(PrintOptions.Compact);

    /// <summary>
    ///  Returns a human-readable representation of this <see cref="AddOrderMessage"/>.
    /// </summary>
