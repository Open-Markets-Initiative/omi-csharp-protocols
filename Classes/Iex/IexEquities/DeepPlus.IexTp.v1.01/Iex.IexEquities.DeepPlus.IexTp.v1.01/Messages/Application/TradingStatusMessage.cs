...
/// <summary>
///  Represents the TradingStatusMessage from the DeepPlus protocol.
/// </summary>
/// <remarks>
...
public sealed class TradingStatusMessage : IMessage, IEquatable<TradingStatusMessage>
{
    /// <summary>
    ///  Message type identifier.
    /// </summary>
    public const char MessageType = 'H';

    /// <summary>
    ///  Total fixed byte length of this message.
    /// </summary>
    private const int MessageByteLength = 21;

    /// <summary>
    ///  Trading status identifier
    /// </summary>
    public TradingStatusField TradingStatus { get; set; } = new();
    ...
    /// </summary>
    public ReasonField Reason { get; set; } = new();
    ...
    /// <summary>
    ///  The total fixed byte length of this message (21 bytes).
    /// </summary>
    ...
    /// <summary>
    ///  Initializes a new <see cref="TradingStatusMessage"/> with all fields at their default values.
    /// </summary>
    public TradingStatusMessage() { }

    /// <summary>
    ///  Initializes a new <see cref="TradingStatusMessage"/> from an existing instance, copying all field values.
    /// </summary>
    /// <param name="other">The source instance to copy from.</param>
    public TradingStatusMessage(TradingStatusMessage other) : this()
    {
        ArgumentNullException.ThrowIfNull(other);
        if (other.Raw is { } raw) Raw = raw.ToArray();
        IsDecoded = other.IsDecoded;
        TradingStatus.CopyFrom(other.TradingStatus);
        Timestamp.CopyFrom(other.Timestamp);
        Symbol.CopyFrom(other.Symbol);
        Reason.CopyFrom(other.Reason);
    }

    /// <summary>
    ///  Parses a <see cref="TradingStatusMessage"/> from a binary data span at the given offset.
    ...
    public TradingStatusMessage(ReadOnlySpan<byte> data, ref int offset)
    {
        var start = offset;
        var decoded = true;
    ...
    /// <summary>
    ///  Parses a <see cref="TradingStatusMessage"/> from a binary data span.
    /// </summary>
    /// <param name="data">The raw message bytes. Must contain at least 21 bytes.</param>
    /// <returns>A new <see cref="TradingStatusMessage"/> with all fields populated from the binary data.</returns>
    public static TradingStatusMessage Parse(ReadOnlySpan<byte> data)
    {
        var offset = 0;
        return new TradingStatusMessage(data, ref offset);
    }

    /// <summary>
    ///  Parses binary data into this <see cref="TradingStatusMessage"/> instance in place, reusing existing field objects.
    /// </summary>
    /// <param name="data">The raw message bytes. Must contain at least 21 bytes.</param>
    ...
    /// <summary>
    ///  Encodes this <see cref="TradingStatusMessage"/> into a binary data span starting at the given offset.
    /// </summary>
    /// <param name="data">The destination span. Must be at least offset + 21 bytes.</param>
    ...
    {
        offset = TradingStatus.Encode(data, offset);
        offset = Timestamp.Encode(data, offset);
        offset = Symbol.Encode(data, offset);
        offset = Reason.Encode(data, offset);
        return offset;
    }

    /// <summary>
    ///  Encodes this <see cref="TradingStatusMessage"/> into a binary data span starting at offset 0.
    /// </summary>
    /// <param name="data">The destination span. Must be at least 21 bytes.</param>
    /// <returns>The offset after the last written byte.</returns>
    public int Encode(Span<byte> data) => Encode(data, 0);

    /// <summary>
    ///  Returns a <see cref="TradingStatusMessage"/> with deterministic sample values in every field,
    ///  suitable for round-trip encoding tests.
    /// </summary>
    public static TradingStatusMessage Sample()
    {
        var msg = new TradingStatusMessage();
        msg.TradingStatus = TradingStatusField.Sample(seed: 0);
        msg.Timestamp = TimestampField.Sample(seed: 1);
        msg.Symbol = SymbolField.Sample(seed: 2);
        msg.Reason = ReasonField.Sample(seed: 3);
        return msg;
    }
    ...
    /// <summary>
    ///  Resets all fields of this <see cref="TradingStatusMessage"/> to their default values and clears the raw backing store.
    /// </summary>
    public void Clear()
    {
        Raw = null;
    ...
    /// <summary>
    ///  Compares this <see cref="TradingStatusMessage"/> to another using field-by-field comparison.
    /// </summary>
    public bool Equals(TradingStatusMessage? other)
    {
        if (other is null) return false;
        if (!TradingStatus.Equals(other.TradingStatus)) return false;
    ...
    /// <summary>
    ///  Compares two <see cref="TradingStatusMessage"/> instances field by field.
    /// </summary>
    public override bool Equals(object? obj)
    {
        return obj is TradingStatusMessage other && Equals(other);
    }
    ...
    /// <summary>
    ///  Returns a hash code for this <see cref="TradingStatusMessage"/> by combining the hash of each field and counted-group container.
    /// </summary>
    ...
    public static bool operator ==(TradingStatusMessage? a, TradingStatusMessage? b) => ReferenceEquals(a, b) || (a is not null && a.Equals(b));
    public static bool operator !=(TradingStatusMessage? a, TradingStatusMessage? b) => !(a == b);
    ...
    public readonly struct Changes : IEquatable<Changes>
    {
        ...
        public override string ToString()
        {
            ...
            var first = true;
            if (Has(0)) { if (!first) sb.Append(", "); sb.Append("TradingStatus"); first = false; }
            if (Has(1)) { if (!first) sb.Append(", "); sb.Append("Timestamp"); first = false; }
            if (Has(2)) { if (!first) sb.Append(", "); sb.Append("Symbol"); first = false; }
            if (Has(3)) { if (!first) sb.Append(", "); sb.Append("Reason"); first = false; }
            return sb.ToString();
        }
    }
    ...
    public Changes Diff(TradingStatusMessage other)
    {
        ArgumentNullException.ThrowIfNull(other);
        var diff = Changes.None;
        if (!TradingStatus.Equals(other.TradingStatus)) diff = diff.With(0);
        if (!Timestamp.Equals(other.Timestamp)) diff = diff.With(1);
        if (!Symbol.Equals(other.Symbol)) diff = diff.With(2);
        if (!Reason.Equals(other.Reason)) diff = diff.With(3);
        return diff;
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(other);
        if (other is not TradingStatusMessage typed) throw new ArgumentException("DiffFields requires same message type", nameof(other));
    ...
    public IEnumerable<(string Name, string Before, string After)> DiffReport(TradingStatusMessage other)
    {
        ArgumentNullException.ThrowIfNull(other);
        var changes = Diff(other);
        if (changes.IsNone) yield break;
        if (changes.Has(0)) yield return (TradingStatus.Name, TradingStatus.ToFormattedString(), other.TradingStatus.ToFormattedString());
        ...
        if (changes.Has(3)) yield return (Reason.Name, Reason.ToFormattedString(), other.Reason.ToFormattedString());
    }
    ...
    {
        FieldName.TradingStatus => TradingStatus,
    ...
    /// <summary>
    ///  Appends a human-readable representation of all fields in this <see cref="TradingStatusMessage"/> to <paramref name="builder"/>,
    ...
    public void ToFormattedString(StringBuilder builder, PrintOptions options = default)
    {
        ...
        builder.Append(indent).AppendLine("TradingStatusMessage:");
        foreach (var f in Fields)
        {
    ...
    /// <summary>
    ///  Returns a human-readable representation of all fields in this <see cref="TradingStatusMessage"/>,
    ...
    public string ToFormattedString(PrintOptions options = default)
    {
        if (options.IsIdentifier)
            return $"[{MessageType}] [{TradingStatus.Value}] [{Timestamp.Value}] [{Symbol.Value}]";
        if (options.IsCompact)
        {
            var sb = new StringBuilder();
            sb.Append("TradingStatusMessage { ");
            bool first = true;
            foreach (var f in Fields)
            {
                if (!first) sb.Append(", ");
    ...
    /// <summary>
    ///  Returns a compact one-line representation of all fields in this <see cref="TradingStatusMessage"/>.
    /// </summary>
    public string ToCompact() => ToFormattedString(PrintOptions.Compact);

    /// <summary>
    ///  Returns a human-readable representation of this <see cref="TradingStatusMessage"/>.
    /// </summary>
