...
/// <summary>
///  Represents the OrderModifyMessage from the DeepPlus protocol.
/// </summary>
/// <remarks>
...
public sealed class OrderModifyMessage : IMessage, IEquatable<OrderModifyMessage>
{
    /// <summary>
    ///  Message type identifier.
    /// </summary>
    public const char MessageType = 'M';
    ...
    ///  Modify Flags
    /// </summary>
    public ModifyFlagsField ModifyFlags { get; set; } = new();
    ...
    /// <summary>
    ///  Initializes a new <see cref="OrderModifyMessage"/> with all fields at their default values.
    /// </summary>
    public OrderModifyMessage() { }

    /// <summary>
    ///  Initializes a new <see cref="OrderModifyMessage"/> from an existing instance, copying all field values.
    /// </summary>
    /// <param name="other">The source instance to copy from.</param>
    public OrderModifyMessage(OrderModifyMessage other) : this()
    {
        ArgumentNullException.ThrowIfNull(other);
        if (other.Raw is { } raw) Raw = raw.ToArray();
        IsDecoded = other.IsDecoded;
        ModifyFlags.CopyFrom(other.ModifyFlags);
    ...
    /// <summary>
    ///  Parses a <see cref="OrderModifyMessage"/> from a binary data span at the given offset.
    ...
    public OrderModifyMessage(ReadOnlySpan<byte> data, ref int offset)
    {
        var start = offset;
        var decoded = true;
    ...
    /// <summary>
    ///  Parses a <see cref="OrderModifyMessage"/> from a binary data span.
    /// </summary>
    /// <param name="data">The raw message bytes. Must contain at least 37 bytes.</param>
    /// <returns>A new <see cref="OrderModifyMessage"/> with all fields populated from the binary data.</returns>
    public static OrderModifyMessage Parse(ReadOnlySpan<byte> data)
    {
        var offset = 0;
        return new OrderModifyMessage(data, ref offset);
    }

    /// <summary>
    ///  Parses binary data into this <see cref="OrderModifyMessage"/> instance in place, reusing existing field objects.
    /// </summary>
    /// <param name="data">The raw message bytes. Must contain at least 37 bytes.</param>
    ...
    /// <summary>
    ///  Encodes this <see cref="OrderModifyMessage"/> into a binary data span starting at the given offset.
    /// </summary>
    ...
    {
        offset = ModifyFlags.Encode(data, offset);
    ...
    /// <summary>
    ///  Encodes this <see cref="OrderModifyMessage"/> into a binary data span starting at offset 0.
    /// </summary>
    ...
    /// <summary>
    ///  Returns a <see cref="OrderModifyMessage"/> with deterministic sample values in every field,
    ///  suitable for round-trip encoding tests.
    /// </summary>
    public static OrderModifyMessage Sample()
    {
        var msg = new OrderModifyMessage();
        msg.ModifyFlags = ModifyFlagsField.Sample(seed: 0);
    ...
    /// <summary>
    ///  Resets all fields of this <see cref="OrderModifyMessage"/> to their default values and clears the raw backing store.
    /// </summary>
    public void Clear()
    {
        Raw = null;
    ...
    /// <summary>
    ///  Compares this <see cref="OrderModifyMessage"/> to another using field-by-field comparison.
    /// </summary>
    public bool Equals(OrderModifyMessage? other)
    {
        if (other is null) return false;
        if (!ModifyFlags.Equals(other.ModifyFlags)) return false;
    ...
    /// <summary>
    ///  Compares two <see cref="OrderModifyMessage"/> instances field by field.
    /// </summary>
    public override bool Equals(object? obj)
    {
        return obj is OrderModifyMessage other && Equals(other);
    }
    ...
    /// <summary>
    ///  Returns a hash code for this <see cref="OrderModifyMessage"/> by combining the hash of each field and counted-group container.
    /// </summary>
    public override int GetHashCode()
    {
        var hash = new HashCode();
        hash.Add(ModifyFlags);
        hash.Add(Timestamp);
        hash.Add(Symbol);
    ...
    public static bool operator ==(OrderModifyMessage? a, OrderModifyMessage? b) => ReferenceEquals(a, b) || (a is not null && a.Equals(b));
    public static bool operator !=(OrderModifyMessage? a, OrderModifyMessage? b) => !(a == b);
    ...
    public readonly struct Changes : IEquatable<Changes>
    {
        ...
        public override string ToString()
        {
            ...
            var first = true;
            if (Has(0)) { if (!first) sb.Append(", "); sb.Append("ModifyFlags"); first = false; }
    ...
    public Changes Diff(OrderModifyMessage other)
    {
        ArgumentNullException.ThrowIfNull(other);
        var diff = Changes.None;
        if (!ModifyFlags.Equals(other.ModifyFlags)) diff = diff.With(0);
    ...
    {
        ArgumentNullException.ThrowIfNull(other);
        if (other is not OrderModifyMessage typed) throw new ArgumentException("DiffFields requires same message type", nameof(other));
    ...
    public IEnumerable<(string Name, string Before, string After)> DiffReport(OrderModifyMessage other)
    {
        ArgumentNullException.ThrowIfNull(other);
        var changes = Diff(other);
        if (changes.IsNone) yield break;
        if (changes.Has(0)) yield return (ModifyFlags.Name, ModifyFlags.ToFormattedString(), other.ModifyFlags.ToFormattedString());
    ...
    {
        FieldName.ModifyFlags => ModifyFlags,
    ...
    /// <summary>
    ///  Appends a human-readable representation of all fields in this <see cref="OrderModifyMessage"/> to <paramref name="builder"/>,
    ...
    public void ToFormattedString(StringBuilder builder, PrintOptions options = default)
    {
        ...
        builder.Append(indent).AppendLine("OrderModifyMessage:");
        foreach (var f in Fields)
        {
    ...
    /// <summary>
    ///  Returns a human-readable representation of all fields in this <see cref="OrderModifyMessage"/>,
    ...
    public string ToFormattedString(PrintOptions options = default)
    {
        if (options.IsIdentifier)
            return $"[{MessageType}] [{ModifyFlags.Value}] [{Timestamp.Value}] [{Symbol.Value}]";
        if (options.IsCompact)
        {
            var sb = new StringBuilder();
            sb.Append("OrderModifyMessage { ");
            bool first = true;
            foreach (var f in Fields)
            {
                if (!first) sb.Append(", ");
    ...
    /// <summary>
    ///  Returns a compact one-line representation of all fields in this <see cref="OrderModifyMessage"/>.
    /// </summary>
    public string ToCompact() => ToFormattedString(PrintOptions.Compact);

    /// <summary>
    ///  Returns a human-readable representation of this <see cref="OrderModifyMessage"/>.
    /// </summary>
